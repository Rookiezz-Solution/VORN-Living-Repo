    ...
    (response) => {
        if (typeof window !== 'undefined') {
            ...
            if (response && response.config && response.config.method !== 'get') {
                window.dispatchEvent(new CustomEvent('toast:show', { detail: { type: 'success', message: 'Request completed successfully' } }));
            }
        }
        return response;
    },
...
export const getCategories = async () => {
    try {
        ...
        console.warn("API call to /categories failed. Using mock data.", error.message);
...
const normalizeProduct = (p) => {
    ...
    const images = (Array.isArray(p.images) ? p.images : (p.ImageURL ? [p.ImageURL] : [])).map(toAbsolute);
    const descriptionHtml = p.FullDescription ?? p.DescriptionHTML ?? null;
    const descriptionText = p.ShortDescription 
        ?? p.Description 
        ...
        ?? p.Overview 
        ?? '';
    return { ...p, images, ImageURL: toAbsolute(p.ImageURL), description: descriptionText, descriptionHtml };
};
...
export const getProducts = async (options = {}) => {
    try {
        ...
        console.warn("API call to /products/filter failed. Using mock data.", error.message);
...
export const getProductById = async (id) => {
    try {
        ...
        return normalizeProduct(response.data);
         ...
         console.warn(`API call to /products/${id} failed. Using mock data.`, error.message);
...
export const getAddresses = async (page = 1, limit = 10) => {
    try {
        const response = await api.get(`/profile/addresses?page=${page}&limit=${limit}`);
        const data = response.data;
        if (Array.isArray(data)) {
            return { items: data, totalCount: data.length, page: 1, limit: data.length };
        }
        return data;
    } catch (error) {
...
export const checkProductAvailability = async (productId, pincode) => {
    try {
        const response = await api.get(`/products/${productId}/availability`, { params: { pincode } });
        return response.data;
    } catch (error) {
...
const getGuestToken = () => {
    ...
    if (!token) {
        token = 'guest_' + Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
...
export const updateCartItem = async (cartItemId, quantity, cartId) => {
    try {
        const response = await api.put(`/cart/item/${cartItemId}`, { quantity, cartId });
        window.dispatchEvent(new Event('cartUpdated')); // Notify listeners
        return response.data;
    } catch (error) {
...
export const removeCartItem = async (cartItemId, cartId) => {
    try {
        const response = await api.delete(`/cart/item/${cartItemId}?cartId=${cartId}`);
        window.dispatchEvent(new Event('cartUpdated')); // Notify listeners
        return response.data;
    } catch (error) {
...
export const removeFromWishlist = async (userId, productId) => {
    try {
        const response = await api.delete(`/wishlist/remove?userId=${userId}&productId=${productId}`);
        return response.data;
    } catch (error) {
...
export const createRazorpayOrder = async (amount) => {
    try {
        ...
        const msg = error?.response?.data?.error || error?.response?.data?.message || error?.message || 'Order creation failed';
        throw new Error(msg);
    }
};
...
export const verifyRazorpayPayment = async ({ razorpay_order_id, razorpay_payment_id, razorpay_signature }) => {
    try {
        const response = await api.post('/verify-payment', { razorpay_order_id, razorpay_payment_id, razorpay_signature });
        return response.data;
    } catch (error) {
...
export const adminAddProductImages = async (id, images, startOrder = 1) => {
    const response = await api.post(`/admin/products/${id}/images`, { images, startOrder });
    return response.data;
};
...
export const adminReorderImage = async (imageId, displayOrder) => {
    const response = await api.put(`/admin/product-images/${imageId}/order`, { displayOrder });
    return response.data;
};
...
export const adminAddProductHighlights = async (id, highlights, replace = false) => {
    const response = await api.post(`/admin/products/${id}/highlights`, { highlights, replace });
    return response.data;
};

export const adminAddProductSpecifications = async (id, specifications, replace = false) => {
    const response = await api.post(`/admin/products/${id}/specifications`, { specifications, replace });
    return response.data;
};

export const adminUploadProductImages = async (id, imagesBase64, startOrder = 1) => {
    const response = await api.post(`/admin/products/${id}/images/upload`, { imagesBase64, startOrder });
    return response.data;
};

export const adminUploadProductVideos = async (id, videosBase64) => {
    const response = await api.post(`/admin/products/${id}/videos/upload`, { videosBase64 });
    return response.data;
};

export const adminAddProductAttributes = async (id, attributes) => {
    const response = await api.post(`/admin/products/${id}/attributes`, { attributes });
    return response.data;
};

export const adminAddProductVariants = async (id, variants) => {
    const response = await api.post(`/admin/products/${id}/variants`, { variants });
    return response.data;
};
...
export const adminUploadCategoryImage = async (id, imageBase64) => {
    const response = await api.post(`/admin/categories/${id}/image/upload`, { imageBase64 });
    return response.data;
};
...
export const adminSetReturnStatus = async (id, action, remarks) => {
    const response = await api.put(`/admin/returns/${id}/status`, { action, remarks });
    return response.data;
};

// Customer replacement selection
export const selectReplacement = async (orderId, orderItemId, { productId, variantId, mode }) => {
    const params = mode ? { mode } : undefined;
    const response = await api.post(`/profile/orders/${orderId}/items/${orderItemId}/replace-select`, { productId, variantId }, { params });
    return response.data;
};
...
export const submitOrderItemReview = async (orderId, orderItemId, { rating, title, description, photoBase64 }) => {
    const response = await api.post(`/profile/orders/${orderId}/items/${orderItemId}/review`, { rating, title, description, photoBase64 });
    return response.data;
};
...
export const adminDashboardSummary = async (days = 30) => {
    const response = await api.get('/admin/dashboard/summary', { params: { days } });
    return response.data;
};
...
export const adminDashboardTopProducts = async (days = 30) => {
    const response = await api.get('/admin/dashboard/top-products', { params: { days } });
    return response.data;
};
