...
const Header = () => {
    ...
    const navItemClass = (path) => {
        ...
        return `rounded-full px-3 py-2 ${isActive ? 'bg-primary text-secondary font-bold' : 'hover:bg-white/10'} transition`;
    };
                ...
                <div className={[headerSurfaceClass, headerTextClass].join(' ')}>
                    ...
                    <div className="container mx-auto py-0 flex justify-between items-center relative">
                        ...
                        <div className={['flex items-center space-x-3', headerTextClass].join(' ')}>
                            {user ? (
                                <Link to="/profile" className={['flex items-center space-x-2 rounded-full px-3 py-2 transition', location.pathname === '/profile' ? 'bg-primary text-secondary font-bold' : pillBtnClass].join(' ')}>
                                    <User className="h-6 w-6" />
                                    <span className="hidden lg:inline">{user.FullName ? user.FullName.split(' ')[0] : 'Profile'}</span>
                                </Link>
                            ) : (
                                <Link to="/auth" className={['flex items-center space-x-2 rounded-full px-3 py-2 transition', location.pathname === '/auth' ? 'bg-primary text-secondary font-bold' : pillBtnClass].join(' ')}>
                                    <User className="h-6 w-6" />
                                    <span className="hidden lg:inline">Login</span>
                                </Link>
                            )}
                            <Link to="/wishlist" className={['relative rf-icon-btn', location.pathname === '/wishlist' ? 'bg-primary text-secondary' : iconBtnClass].join(' ')}>
                                <Heart className="h-6 w-6" />
                            </Link>
                            <Link to="/cart" className={['relative rf-icon-btn', location.pathname === '/cart' ? 'bg-primary text-secondary' : iconBtnClass].join(' ')}>
                                <ShoppingCart className="h-6 w-6" />
                                {cartCount > 0 && (
                                    <span className={`absolute -top-2 -right-2 ${location.pathname === '/cart' ? 'bg-white text-secondary' : 'bg-primary text-secondary'} text-xs rounded-full h-5 w-5 flex items-center justify-center font-semibold`}>
                                        {cartCount}
                                    </span>
                                )}
                            </Link>
                    ...
                    <nav className={['hidden md:block border-t border-white/10', headerTextClass].join(' ')}>
                        <div className="container mx-auto flex gap-8 py-3 font-medium items-center">
                            ...
                            >
                                <button
                                    className={['flex items-center focus:outline-none', location.pathname.startsWith('/category/') ? 'bg-primary text-secondary font-bold rounded-full px-3 py-2' : 'rounded-full px-3 py-2 hover:bg-white/10 transition'].join(' ')}
                                    onMouseEnter={() => {
                                        isHeaderHiddenRef.current = false;
                                        setIsHeaderHidden(false);
                    ...
                    {isMenuOpen && (
                        <div className="md:hidden bg-black/55 backdrop-blur-2xl border-t border-white/10 p-4 absolute w-full shadow-lg z-50 text-white">
                            <div className="flex flex-col space-y-4">
                                <Link to="/" className={`rounded-full px-3 py-2 ${location.pathname === '/' ? 'bg-primary text-secondary font-bold' : 'hover:bg-black/10'} transition`} onClick={() => setIsMenuOpen(false)}>Home</Link>
                                <Link to="/shop" className={`rounded-full px-3 py-2 ${location.pathname === '/shop' ? 'bg-primary text-secondary font-bold' : 'hover:bg-black/10'} transition`} onClick={() => setIsMenuOpen(false)}>Shop</Link>
                                ...
                                <div>
                                    <button
                                        ...
                                        className={`flex items-center justify-between w-full rounded-full px-3 py-2 ${location.pathname.startsWith('/category/') ? 'bg-primary text-secondary font-bold' : 'hover:bg-white/10'} transition`}
                                    >
                                        ...
                                        <div className="pl-4 mt-2 space-y-2 border-l-2 border-primary ml-2 max-h-60 overflow-y-auto">
                                            {categories.map((category) => (
                                                <Link
                                                    key={category.CategoryID}
                                                    to={`/category/${category.CategorySlug}`}
                                                    className={`block text-sm ${location.pathname === `/category/${category.CategorySlug}` ? 'text-primary font-bold' : 'text-white/90 hover:bg-white/10'} rounded-lg px-2 py-1 transition`}
                                ...
                                <Link to="/about" className={`rounded-full px-3 py-2 ${location.pathname === '/about' ? 'bg-primary text-secondary font-bold' : 'hover:bg-white/10'} transition`} onClick={() => setIsMenuOpen(false)}>About</Link>
                                <Link to="/contact" className={`rounded-full px-3 py-2 ${location.pathname === '/contact' ? 'bg-primary text-secondary font-bold' : 'hover:bg-white/10'} transition`} onClick={() => setIsMenuOpen(false)}>Contact</Link>
                            </div>
                        </div>
                    )}
                </div>
