...
const placeOrder = async (req, res) => {
    const { items, address, addressId, paymentMethod, userId, payment } = req.body;
    ...
    try {
        ...
        if (pm === 'RAZORPAY' || pm === 'ONLINE') {
            ...
            paymentStatus = 'Paid';
            razorpayMeta = { razorpay_order_id, razorpay_payment_id, razorpay_signature };
        }

        // 1. Resolve Address (reuse if provided or identical exists)
        let shippingAddressId = addressId;
        if (!shippingAddressId) {
            shippingAddressId = await orderModel.findOrCreateAddress({ ...address, userId });
        }
        ...
        if (error && (error.code === 'INVALID_PRODUCT_ID' || error.code === 'INVALID_QUANTITY' || error.code === 'INVALID_VARIANT_ID')) {
            return res.status(400).json({ message: "Invalid order payload. Please refresh your cart and try again." });
        }
        if (error && (error.code === 'INSUFFICIENT_STOCK' || error.code === 'INSUFFICIENT_VARIANT_STOCK')) {
            return res.status(409).json({ message: "Insufficient stock for one or more items. Please refresh and try again." });
        }
...
const requestReplacement = async (req, res) => {
    try {
        const userId = req.body.userId || req.user?.UserID;
        const { id } = req.params;
        let { orderItemId, reason, reasonCategory, imageUrl, imageBase64 } = req.body;
        ...
        const result = await orderModel.requestReplacement(userId, id, orderItemId, { reason, reasonCategory, imageUrl });
        return res.status(200).json({ message: 'Replacement requested', ...result });
    } catch (error) {
        if (error.code === 'ORDER_NOT_FOUND') {
            return res.status(404).json({ message: 'Order not found' });
        }
