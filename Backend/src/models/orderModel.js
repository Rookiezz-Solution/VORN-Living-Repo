...
const createOrder = async (orderData) => {
    ...
    try {
        ...
        const [hasPaymentMethod, hasRzpOrder, hasRzpPayment, hasRzpSig] = await Promise.all([
...
const findOrCreateAddress = async (address) => {
    try {
        ...
        request.input('userId', address.userId); // Can be null
        request.input('fullName', address.name);
        request.input('phoneNumber', address.phone);
        ...
        const result = await request.query(`
            INSERT INTO User_Addresses (UserID, FullName, PhoneNumber, AddressLine1, City, State, PinCode, Country)
            OUTPUT INSERTED.AddressID
            VALUES (@userId, @fullName, @phoneNumber, @addressLine1, @city, @state, @pinCode, @country)
        `);
